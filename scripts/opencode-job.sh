...
# Installed to $PREFIX/bin/termux-opencode-job by install.sh. Manages headless
# `opencode run` jobs scheduled through Android's real JobScheduler (via
...
usage() {
  cat <<'EOF'
usage:
  termux-opencode-job add <name> --prompt "TEXT" [--prompt-file PATH]
                     [--period-ms N] [--cwd DIR] [--persisted]
                     [--charging] [--network any|unmetered|cellular|not_roaming|none]
      Schedule a headless `opencode run` job. Re-running add with the same
...
cmd_add() {
  ...
  printf '#!/data/data/com.termux/files/usr/bin/bash\nexec %q %q\n' "$DEST/opencode-job-runner.sh" "$name" > "$stub"
  chmod 700 "$stub"
...
cmd_run() {
  ...
  "$DEST/opencode-job-runner.sh" "$name"
}
